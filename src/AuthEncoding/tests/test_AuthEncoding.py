...
@pytest.mark.parametrize(
    'password', ['good_pw', 'gööd_pw', 'gööd_pw'.encode('latin-1')])
def testGoodPassword(schema_id, password):
    ...
    assert AuthEncoding.pw_validate(AuthEncoding.text(enc), password)
...
def testBadPassword(schema_id, password):
    ...
    assert not AuthEncoding.pw_validate(AuthEncoding.text(enc), 'xxx')
